...
export class Q3AgentViewPane extends ViewPane {
	...
	private _formatToolApproval(toolName: string, toolArgs: string): { title: string; details: [string, string][]; preview?: string; actionLabel: string } {
		let args: any = {};
		...
		switch (toolName) {
			case 'apply_edit': {
				...
				if (args.replace_all) {
					details.push(['Mode', 'Replace all occurrences']);
				}
				return { title: `Edit file: \`${filePath}\``, details, actionLabel: 'Approve' };
			}
			case 'batch_edit': {
				const filePath = args.path || args.file_path || '<unknown>';
				const editCount = args.edits?.length || 0;
				details.push(['File', filePath]);
				details.push(['Edits', String(editCount)]);
				return { title: `Batch edit file: \`${filePath}\` (${editCount} edits)`, details, actionLabel: 'Approve' };
			}
			case 'write_file': {
				const filePath = args.path || args.file_path || '<unknown>';
				details.push(['File', filePath]);
				if (args.content) {
					const lang = this._getLanguageFromPath(filePath);
					const content = args.content.length > 1000 ? args.content.substring(0, 997) + '\n...' : args.content;
					preview = `${lang}\n${content}`;
				}
				return { title: `Write file: \`${filePath}\``, details, preview, actionLabel: 'Approve' };
			}
			case 'run_command': {
				const cmd = args.command || '<unknown>';
				details.push(['Command', cmd]);
				preview = `bash\n${cmd}`;
				return { title: 'Run terminal command', details, preview, actionLabel: 'Run' };
			}
			case 'git_commit': {
				const msg = args.message || '<unknown>';
				details.push(['Message', msg]);
				preview = `bash\ngit add -A && git commit -m "${msg}"`;
				return { title: 'Git commit (stage all + commit)', details, preview, actionLabel: 'Approve' };
			}
			case 'read_file': {
				const filePath = args.path || '<unknown>';
				details.push(['File', filePath]);
				return { title: `Read file: \`${filePath}\``, details, actionLabel: 'Allow' };
			}
			case 'list_dir': {
				const dirPath = args.path || '<unknown>';
				details.push(['Directory', dirPath]);
				return { title: `List directory: \`${dirPath}\``, details, actionLabel: 'Allow' };
			}
			case 'grep_search': {
				details.push(['Pattern', args.pattern || '<unknown>']);
				details.push(['Path', args.path || '<unknown>']);
				if (args.include) { details.push(['Filter', args.include]); }
				return { title: 'Search files', details, actionLabel: 'Allow' };
			}
			case 'git_status': {
				return { title: 'Check git status', details, actionLabel: 'Allow' };
			}
			case 'read_diagnostics': {
				return { title: 'Read editor diagnostics', details, actionLabel: 'Allow' };
			}
			default: {
				for (const [k, v] of Object.entries(args)) {
					const val = typeof v === 'string' ? (v.length > 80 ? v.substring(0, 77) + '...' : v) : String(v);
					details.push([k, val]);
				}
				return { title: `Approve ${toolName}?`, details, actionLabel: 'Approve' };
			}
		}
	}
	...
	private _handleChunk(chunk: IQ3AgentResponseChunk): void {
		...
		this._removeThoughtIndicator();
			...
			const wrapper = document.createElement('div');
			wrapper.classList.add('q3-agent-approval');

			const desc = this._formatToolApproval(chunk.toolName || '', chunk.toolArgs || '');

			const label = document.createElement('div');
			label.classList.add('q3-agent-approval-label');
			this._renderMarkdownInto(label, desc.title);
			wrapper.appendChild(label);

			if (desc.details.length > 0) {
				const detailList = document.createElement('div');
				detailList.classList.add('q3-agent-approval-details');
				for (const [key, value] of desc.details) {
					const row = document.createElement('div');
					row.classList.add('q3-agent-approval-detail-row');

					const keyEl = document.createElement('span');
					keyEl.classList.add('q3-agent-approval-detail-key');
					keyEl.textContent = `${key}:`;
					row.appendChild(keyEl);

					const valEl = document.createElement('span');
					valEl.classList.add('q3-agent-approval-detail-val');
					this._renderMarkdownInto(valEl, '`' + value + '`');
			...
			if (desc.preview) {
				...
				preview.classList.add('q3-agent-approval-preview');
				this._renderMarkdownInto(preview, '```' + desc.preview + '\n```');
				wrapper.appendChild(preview);
			}

			// For apply_edit, render a colored diff table instead of plain text preview
			if (chunk.toolName === 'apply_edit') {
				let parsedArgs: any = {};
				try { parsedArgs = JSON.parse(chunk.toolArgs || '{}'); } catch {}
				if (parsedArgs.old_string) {
					const diffTable = this._renderDiffTable(parsedArgs.path || '', parsedArgs.old_string || '', parsedArgs.new_string || '');
					wrapper.appendChild(diffTable);
					this._showInlineDiffInEditor(parsedArgs.path || '', parsedArgs.old_string || '', parsedArgs.new_string || '', chunk.toolCallId || '');
				}
			}

			// For write_file, also show inline diff in editor
			if (chunk.toolName === 'write_file') {
				let parsedArgs: any = {};
				try { parsedArgs = JSON.parse(chunk.toolArgs || '{}'); } catch {}
				if (parsedArgs.content) {
					this._showInlineDiffInEditor(parsedArgs.path || parsedArgs.file_path || '', '', parsedArgs.content || '', chunk.toolCallId || '');
				}
			}

			// For batch_edit, show summary of edits
			if (chunk.toolName === 'batch_edit') {
				let parsedArgs: any = {};
				try { parsedArgs = JSON.parse(chunk.toolArgs || '{}'); } catch {}
				if (parsedArgs.edits && parsedArgs.edits.length > 0) {
					const summary = document.createElement('div');
					summary.classList.add('q3-agent-batch-edit-summary');
					summary.textContent = parsedArgs.edits.length + ' edits to ' + (parsedArgs.path || 'file');
			...
			approveBtn.classList.add('q3-agent-approve-btn');
			approveBtn.textContent = desc.actionLabel;
			approveBtn.addEventListener('click', () => {
				this._agentService.resolveApproval(chunk.toolCallId!, true);
				this._inlineDiffController?.clearAll();
				wrapper.remove();
			});
			btnRow.appendChild(approveBtn);

			const rejectBtn = document.createElement('button');
			rejectBtn.classList.add('q3-agent-reject-btn');
