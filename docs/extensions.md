...
Being a vscode based editor, Q3 IDE gets additional features by installing Visual Studio Code extensions.
...
As noted above, the [Open VSX Registry](https://open-vsx.org/) is the pre-set extension gallery in Q3 IDE. Using the extension view in Q3 IDE will therefore by default use it.
...
Or by creating a custom `product.json` at the following location (replace `Q3 IDE` by `Q3 IDE - Insiders` if you use that):
...
* [Open VSX](https://github.com/eclipse/openvsx) eclipse open-source project
  While the public instance which is run by the Eclipse Foundation is the pre-set endpoint in Q3 IDE, you can host your own instance.
...
The [**VSIX Manager**](https://github.com/zokugun/vscode-vsix-manager) extension provides a powerful and user-friendly interface for managing `.vsix` files directly within Q3 IDE. Its author is the main maintainer of Q3 IDE ;)
